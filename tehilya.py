...
def visit_rock(player_name, player_hp, player_money, player_xp, player_level, player_telum, player_armor):
    ...
    damage = ''
    if option == '1':
        fight(player_name, player_hp, player_xp, player_level, player_telum, player_armor, damage) 
    elif option == '2':
...
def fight(player_name, player_hp, player_xp, player_level, player_telum, player_armor, damage):
        ...
        if player_hit >= 11:
            ...
            if enemy_hp <= 0:
                ...
                if player_xp >= 1:
                    ...
                    level_change = player_xp // 100 + 1
                    player_level += level_change
                    player_xp -= level_change
                    print('Уровень', player_name, 'равен', player_level)
                    print('Теперь у', player_name, player_xp, 'xp')
                    print('-' * 20)
                    answer = input('Выберете вариант ответа: 1 - вернуться к камню, 2 - начать новый бой, 0 -выйти из игры ')
                    if answer == '1':
                        player_money = ''
                        pause()
                        visit_rock(player_name, player_hp, player_money, player_xp, player_level, player_telum, player_armor)
